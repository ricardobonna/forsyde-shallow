...
eulerSY :: RealFloat a => a -> a -> Signal a -> Signal a
eulerSY dt x0 inp = out
  where out = delaySY x0 middle
        middle = zipWithSY (+) out (mapSY (*dt) inp)


trapezSY :: RealFloat a => a -> a -> Signal a -> Signal a
trapezSY dt x0 inp = out
  where out = delaySY x0 middle
        inp1 = delaySY 0 inp
        middle = zipWithSY (+) out (mapSY (*(dt/2)) (zipWithSY (+) inp inp1))


eulernSY :: RealFloat a => a -> [a] -> Signal [a] -> Signal [a]
eulernSY dt x0 inp = out
  where out = delaySY x0 middle
        middle = zipWithSY (zipWith (+)) out (mapSY (map (*dt)) inp)


trapeznSY :: RealFloat a => a -> [a] -> Signal [a] -> Signal [a]
trapeznSY dt x0 inp = out
  where out = delaySY x0 middle
        inp1 = delaySY (headS inp) (tailS inp)
        middle = zipWithSY (zipWith (+)) out (mapSY (map (*(dt/2))) (zipWithSY (zipWith (+)) (tailS inp) inp1))



firstOrdSys :: Signal Float -> Signal Float
firstOrdSys u = x
  where x = trapezSY 0.1 0 dx
        dx = zipWithSY (-) u x
